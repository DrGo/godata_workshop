...
//
// This is the non-concurrent version of the script, see gcos_monthly.go
...
// and the data files individually gzipped.
//
...
func parse(line string) *lrec_t {
	...
	for pos := 21; pos < len(line); pos += 8 {
		...
		if line[26] != ' ' {
			...
			rec.Valid = append(rec.Valid, false)
			continue
		}
		...
		rec.Valid = append(rec.Valid, true)
	}
...
func process_file(file os.FileInfo) {
